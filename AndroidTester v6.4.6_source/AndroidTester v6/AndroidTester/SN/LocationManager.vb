...
Public Class LocationManager
    ...
    Public Sub New()
        ...
        Me.GoogleMapsButton.Enabled = False
        Me.YandexMapsButton.Enabled = False
    End Sub
    ...
    Private Sub d(ByVal img As Image)
        Me.GoogleMapsButton.Enabled = True
        Me.YandexMapsButton.Enabled = True
    ...
    Private Function GMap(ByVal Latitude As Double, ByVal Longitude As Double, ByVal Zoom As Integer, ByVal h As Integer, ByVal w As Integer, ByVal st As String) As String
        ...
        Return String.Concat(New String() {"https://static-maps.yandex.ru/1.x/?lang=en_US&ll=", Conversions.ToString(Longitude), ",", Conversions.ToString(Latitude), "&z=", Conversions.ToString(Zoom), "&l=", st, "&size=450,450", "&pt=", Conversions.ToString(Longitude), ",", Conversions.ToString(Latitude), ",pm2rdl"})
    End Function
    ...
    Private Sub MapBox_Paint(ByVal sender As Object, ByVal e As PaintEventArgs) Handles MapBox.Paint
        ...
        ElseIf (Me.v) Then
            ...
            e.Graphics.DrawString("Waiting GPS Signal...", font, white, 10.0!, 10.0!)
        End If
    End Sub
    ...
    Private Sub REMap()
        Me.LBLZOOM.Text = String.Concat("Zoom:", Conversions.ToString(Store.RateConverter(Me.zoom, 18)), "%")
        Me.lnk = Me.GMap(Me.Lit, Me.Lon, Me.zoom, 450, 450, If(Operators.ConditionalCompareObjectEqual(Me.BTNSty.Tag, "0", False), "map", "sat"))
        Me.ia = True
    End Sub
    ...
    Public Sub TData(ByVal Ay As Array)
        Try
            ...
            ElseIf (Operators.CompareString(value, Store.BFF(Store.buff, CLng(105)), False) = 0) Then
                If (Not Me.w) Then
                    Me.SaveTitle.Append(String.Concat("<title>Android Tester - ", MyBase.Name, "</title>"))
                    Me.SaveColumns.Append("</tr><th>Date & Time</th><th>Accuracy</th><th>Provider</th><th>Location</th><tr>")
                    Me.w = True
                End If
                Me.v = False
                ...
                If (Operators.CompareString(strArrays(0), "-1", False) <> 0 And Operators.CompareString(strArrays(1), "-1", False) <> 0) Then
                    ...
                    Me.LatText.Text = String.Concat("Lat: ", Me.Lit)
                    Me.LonText.Text = String.Concat("Lon: ", Me.Lon)
                    ...
                    Me.SaveRows.Append(String.Concat(New String() {"</tr> <td>" + DateAndTime.Now.ToString("yyyy-MM-dd_HH.mm.ss") + "</td><td>", str, "</td><td>", str1, "</td><td><a href=""", str2, """>Show On Map&nbsp;</a></td><tr>" & vbCrLf & ""}))
                End If
            ...
            ElseIf (Operators.CompareString(value, Store.BFF(Store.buff, CLng(107)), False) = 0) Then
                ...
                'Console.WriteLine(str6) network, gps, gsm cell, lkl gps
                'Me.PNLERRORS.Visible = True
                Me.LastUpdate.Text = ("Updated: " + DateAndTime.Now.ToString("HH:mm:ss"))
                ...
                If (Versioned.IsNumeric(str3) And Versioned.IsNumeric(str3)) Then
                    ...
                    Me.SaveRows.Append(String.Concat(New String() {"</tr> <td>" + DateAndTime.Now.ToString("yyyy-MM-dd_HH.mm.ss") + "</td><td>", str5, "</td><td>", str6, "</td><td><a href=""", str7, """>Show On Map&nbsp;</a></td><tr>" & vbCrLf & ""}))
                End If
    ...
    Private Sub GoogleMapsLink_Click(sender As Object, e As EventArgs) Handles GoogleMapsButton.Click
        Dim str7 As String = String.Concat(New String() {"https://www.google.com/maps/dir/", Conversions.ToString(Me.Lit), ",", Conversions.ToString(Me.Lon), "/@", Conversions.ToString(Me.Lit), ",", Conversions.ToString(Me.Lon), ",18z"})
        Process.Start(str7)
    End Sub

    Private Sub YandexMaps_Click(sender As Object, e As EventArgs) Handles YandexMapsButton.Click
        Dim str7 As String = String.Concat(New String() {"https://yandex.com/maps/?ll=", Conversions.ToString(Me.Lon), ",", Conversions.ToString(Me.Lit), "&z=18&whatshere%5Bpoint%5D=", Conversions.ToString(Me.Lon), ",", Conversions.ToString(Me.Lit)})
        Process.Start(str7)
    End Sub
