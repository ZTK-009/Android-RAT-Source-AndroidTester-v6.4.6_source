...
Public Class File_Manager
    ...
    Public Sub TData(ByVal ClientData As Array)
        Try
            Dim ClientValue As String
            Try
                ClientValue = CStr(ClientData.GetValue(0))
            Catch exception As System.Exception
            End Try

            If (Operators.CompareString(ClientValue, Store.BFF(Store.buff, CLng(112)), False) = 0) Then
                Me.LBER.Text = CStr(Operators.ConcatenateObject("File transfer completed :", ClientData.GetValue(1)))
                Me.PNLERRORS.Visible = True
                Me.PNLERRORS.Refresh()

            ElseIf (Operators.CompareString(ClientValue, Store.BFF(Store.buff, CLng(83)), False) = 0) Then
                Me.LBER.Text = CStr(ClientData.GetValue(1))
                Me.PNLERRORS.Visible = True
                Me.PNLERRORS.Refresh()

            ElseIf (Operators.CompareString(ClientValue, Store.BFF(Store.buff, CLng(84)), False) = 0) Then
                Dim str As String = Nothing
                Dim strArrays As String() = CStr(ClientData.GetValue(2)).Split(New String() {Data.SplitPaths}, StringSplitOptions.RemoveEmptyEntries)
                ...
                Dim ClientDataString As String() = CStr(ClientData.GetValue(1)).Split(New String() {Data.SplitLines}, StringSplitOptions.RemoveEmptyEntries)

                Dim ClientDataStringLength As Integer = ClientDataString.Length - 1
                For i As Integer = 0 To ClientDataStringLength
                    Dim Seperator As String() = New String() {Data.SplitArray}
                    Dim ClientDataArray As String() = ClientDataString(i).Split(Seperator, StringSplitOptions.RemoveEmptyEntries)
                    '10250 ve 10251 fix
                    If (ClientDataArray.Length <= 3) Or (ClientDataArray.Length > 6) Or (ClientDataArray.Length = 5) Then
                        GoTo Label1
                    End If

                    Dim DataFlag As Boolean = Operators.CompareString(ClientDataArray(0), "-1", False) <> 0 And Operators.CompareString(ClientDataArray(1), "-1", False) <> 0 And Operators.CompareString(ClientDataArray(2), "-1", False) <> 0
                    If DataFlag Then
                        ...
                        'Folder or Not
                        If (Operators.CompareString(ClientDataArray(0), Store.BFF(Store.buff, CLng(111)), False) <> 0) Then
                            Extension = ClientDataArray(0)
                        ...
                        Else
                            ...
                            Try
                                Extension = (New FileInfo(ClientDataArray(1))).Extension
                            Catch exception As System.Exception
                        ...
                        If (isFile) Then
                            FileSize = CStr(BytesConverter(Conversions.ToLong(ClientDataArray(2))).GetValue(0))
                        End If
                        ...
                        Dim str1 As String = ClientDataArray(4)
                        Dim str2 As String = Strings.Space(1)
                        If (Operators.CompareString(str1, "n/a", False) <> 0) Then
                            If (Operators.CompareString(str1.Trim(), ClientDataArray(5).Trim(), False) = 0) Then
                                str2 = " (New)"
                            End If
                        End If

                        'File
                        Me.ViewManager.Rows.Add(New Object() {IMG(DirectCast(Me.imageList_0.Images(num1), Bitmap), Extension), ClientDataArray(1), FileSize, Extension, str1, str2})
                        If (isFile) Then
                            'ViewManager.Rows(ViewManager.Rows.Count - 1).Cells(2).Tag = Conversions.ToLong(ClientDataArray2(2))
                            ViewManager.Rows(ViewManager.Rows.Count - 1).Cells(2).Tag = ClientDataArray(2)
                        End If
                        ...
                        If (Me.indxOld = -1) Then
                            If (Operators.ConditionalCompareObjectEqual(Me.SearchObj(Me.TEXTPATH.Text).GetValue(1), ClientDataArray(1), False)) Then
                                Me.indxOld = Me.ViewManager.Rows(Me.ViewManager.Rows.Count - 1).Index
                            End If
                        End If
                    End If

                    If (Operators.CompareString(str, ClientDataArray(3), False) <> 0) Then
                        str = ClientDataArray(3)
